...
ส่งไฟล์ในโฟลเดอร์ `responsive-form/`
...
เปิดไฟล์ `responsive-form/index.html` ใน browser ได้ทันที

## 2. Logical and AI

สำหรับการโชว์ตอนสัมภาษณ์ ผมเตรียมโปรเจกต์ย่อยไว้ที่ `question2-restaurant-booking-ai-demo/` โดยแยกไฟล์ `AGENTS.md`, `REQUIREMENTS.md`, `SKILL.md`, และ `PROMPT.md` เพื่อให้เปิด new chat แล้วให้ AI ทำงานตาม requirement ได้จริง
...
| `Enrollments` | `EnrollmentId`, `StudentId`, `CourseId`, `EnrolledAt`, `EnrollmentStatus` | PK: `EnrollmentId`, FK: `StudentId`, `CourseId`, Unique: `StudentId + CourseId` | `StudentId`, `CourseId` |
...
ดู DDL เต็มได้ที่ `answers/database_design.sql`
...
ดู script เต็มที่รวมการสร้าง `@tbCustomers` ได้ที่ `answers/cursor_loop.sql`
